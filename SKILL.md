...
description: Use this skill when the user wants structured market research, trend validation, market analysis, or opportunity mapping. Triggers include requests like "research the X market", "validate this trend", "is X hype or real", "competitive landscape of X", "market sizing for X", or any request for a thesis-driven research report with data, competitive analysis, and actionable opportunities. Also use when the user asks for deeper ideation, more creative product ideas, or wants to explore opportunities from existing research.
---
...
After the user selects their preferred opportunities from Deeper Ideation Mode, suggest they develop them further: *"Want me to turn these into full product definitions with architecture, business model, go-to-market, and a 90-day build plan?"*

If the user has the **Business Idea Plan** prompt (a companion 12-dimension product definition framework), use it. If not, you can still produce a structured product plan — just follow a logical structure: problem → user → value prop → features → architecture → business model → go-to-market → risks → 90-day plan.
